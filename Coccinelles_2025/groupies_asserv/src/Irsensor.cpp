...
// Broches I2C personnalisées
const int SDA_PIN = 4;  // D2
const int SCL_PIN = 2;  // D4

// Résolution du capteur
int imageWidth = 8; // 8x8 zones

void setup() {
  Serial.begin(115200);
  delay(500);

  // Initialisation I2C personnalisée
  Wire.begin(SDA_PIN, SCL_PIN);
  Wire.setClock(1000000); // Fréquence I2C recommandée : 1 MHz

  Serial.println("Initialisation du capteur VL53L5CX...");
  ...
  myToF.setResolution(64);      // 8x8
  myToF.setRangingFrequency(10); // 10 Hz (max 15 Hz en 8x8)

  myToF.startRanging();
  Serial.println("Capteur prêt. Lecture des données...");
}

void loop() {
  if (myToF.isDataReady()) {
    if (myToF.getRangingData(&tofData)) {
      Serial.println("------ Nouvelle mesure ------");

      int minDist = 8190; // Valeur max du capteur
      int centerZone = 35; // Zone centrale approximative
      int centerDistance = tofData.distance_mm[centerZone];

      for (int i = 0; i < 64; i++) {
        if (tofData.distance_mm[i] < minDist && tofData.distance_mm[i] > 0) {
          minDist = tofData.distance_mm[i];
        }
      }

      Serial.print("Distance centrale (zone 35) : ");
      ...
      // Détection simple : objet proche ?
      if (centerDistance < 300 && centerDistance > 0) {
        Serial.println("⚠️ Objet détecté à moins de 30 cm !");
      }
